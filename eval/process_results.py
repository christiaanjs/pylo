...
def process_beast_trace(result_filename, config, resample=False, burn_in=True):
    trace_df = pd.read_table(result_filename, comment='#')
    to_use = trace_df[int(trace_df.shape[0]*config['burn_in']):] if burn_in else trace_df
